...
<template>
  <div class="space-y-6">
    ...
    <div v-else class="space-y-6">
      <div v-if="taskId" class="p-4 bg-indigo-50/50 border border-indigo-100 rounded-2xl">
        <h3 class="text-xs font-black text-indigo-400 uppercase tracking-widest mb-3 flex items-center gap-2"><Check :size="14" /> Tag di Tugas Ini</h3>
        <div class="flex flex-wrap gap-2">
          <button
            ...
            class="group relative px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-2 shadow-sm"
          ...
          >
            <span v-if="isTagAssigned(tag.id)" class="w-2 h-2 bg-white rounded-full animate-pulse"></span>
            {{ tag.name }}
          </button>
          <p v-if="!allTags.length" class="text-xs text-slate-400 italic">Belum ada pilihan tag.</p>
        </div>
      </div>
      ...
      <div class="p-4 bg-white border border-slate-200 rounded-2xl shadow-sm space-y-4">
        <div class="flex items-center gap-2 mb-1">
          ...
          <h3 class="text-sm font-bold text-slate-700">Buat Tag Master</h3>
        </div>
        ...
        <div class="space-y-3">
          ...
          <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <button @click="createTag" :disabled="isActionLoading || !newTag.name.trim()" class="px-4 py-2 bg-indigo-600 text-white text-xs font-bold rounded-xl flex items-center gap-2 disabled:opacity-50">
              <Loader2 v-if="isActionLoading" class="animate-spin" :size="14" />
              <Plus v-else :size="14" />
              Tambah
            </button>
          </div>
      ...
      <div class="space-y-3">
        <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] px-1">Manajemen Semua Tag</h3>

        <div v-if="allTags.length" class="grid gap-2">
          <div v-for="tag in allTags" :key="tag.id" class="group flex items-center justify-between p-3 bg-white border border-slate-200 rounded-xl hover:border-indigo-200 transition-all">
            <div v-if="editingTag?.id === tag.id" class="flex-1 flex items-center gap-2">
              <input v-model="editingTag.name" class="flex-1 px-2 py-1 border rounded text-sm" />
              <div class="flex gap-1">
                <button v-for="c in colors" :key="c" class="w-4 h-4 rounded-full" :style="{ backgroundColor: c }" @click="editingTag.color = c"></button>
              </div>
              <button @click="updateTag" class="text-emerald-500 p-1"><Check :size="18" /></button>
              <button @click="editingTag = null" class="text-slate-400 p-1"><X :size="18" /></button>
            </div>

            <template v-else>
              <div class="flex items-center gap-3">
                <div class="w-3 h-3 rounded-full shadow-inner" :style="{ backgroundColor: tag.color }"></div>
                <span class="text-sm font-bold text-slate-700">{{ tag.name }}</span>
              </div>
              <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button @click="editingTag = { ...tag }" class="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-all">
                  <Pencil :size="14" />
                </button>
                <button @click="deleteTag(tag.id)" class="p-1.5 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all">
                  <Trash2 :size="14" />
                </button>
              </div>
            </template>
        ...
        <div v-else class="text-center py-10 border-2 border-dashed border-slate-100 rounded-2xl">
          <Tags class="mx-auto text-slate-200 mb-2" :size="32" />
          <p class="text-xs text-slate-400 font-medium">Belum ada tag dibuat.</p>
        </div>
      </div>
    </div>
  </div>
